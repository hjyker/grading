...
def create_order(music_no, maker=None):
    status = 'unassign' if maker is None else 'unaudit'
    AuditOrder.objects.create(music_no=music_no, maker=maker, status=status)
...
if __name__ == '__main__':
    ...
    from findiff.models import AuditOrder, Comments, Labels, UserProfile

    for music_no, reviews, labels in parse_origin_data('./init_dataset'):
        create_order(music_no, maker=UserProfile.objects.get(id=2))
        create_comment(music_no, reviews)
        create_label(music_no, labels)
        print(f'SUCCESS {music_no} within AuditOrder, Comments, Labels')
